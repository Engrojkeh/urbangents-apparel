  ...
  return (
    <div className="container mt-2">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1.5rem' }}>
        {cartItems.map(item => (
          <div key={item.product_id} style={{ display: 'flex', alignItems: 'center', backgroundColor: 'var(--bg-secondary)', padding: '1rem', borderRadius: '8px', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <button onClick={() => updateQuantity(item.product_id, -1)} style={{ padding: '0.25rem 0.75rem', fontSize: '1.2rem', backgroundColor: '#333', color: 'white', borderRadius: '4px' }}>-</button>
              <span style={{ fontSize: '1.2rem' }}>{item.quantity}</span>
              <button onClick={() => updateQuantity(item.product_id, 1)} style={{ padding: '0.25rem 0.75rem', fontSize: '1.2rem', backgroundColor: '#333', color: 'white', borderRadius: '4px' }}>+</button>
            </div>
            <button onClick={() => removeFromCart(item.product_id)} style={{ backgroundColor: 'var(--danger)', color: 'white', padding: '0.5rem 1rem', borderRadius: '4px' }}>Remove</button>
          </div>
        ))}
      </div>
